...
local micronanofab = {
	...
	--	player:addCredits(MNF:getTraits().ammo)
		...
		local mainframenano = false
			for i,nanunit in pairs(sim:getAllUnits())do
				if nanunit:getTraits() and nanunit:getTraits().storeType and (nanunit:getTraits().storeType=="standard" or nanunit:getTraits().storeType=="large") then
					mainframenano = nanunit
				end
			end --nanunit:getTraits().mainframe_item
		...
		--mainframenano:getTraits().shopDiscount = .9
