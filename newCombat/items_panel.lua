...
function items_panel.loot:refreshItem( widget, i )
	...
	if widget._item then
		...
		if widget._item:getTraits().vaultLocked then
			enabled = false
			reason = "Draw locked with " .. widget._item:getTraits().vaultLocked -- util.sformat(STRINGS.UI.TOOLTIPS.PICK_UP_CONDITION_DESC, util.toupper(item:getTraits().pickupOnly) )
		end
