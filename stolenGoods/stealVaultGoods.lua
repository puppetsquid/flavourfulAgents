...
local stealVaultGoods =
	{
		...
		canUseAbility = function( self, sim, unit, userUnit )
			...
			if unit:getPlayerOwner() ~= userUnit:getPlayerOwner() then  -- and unit:getTraits().mainframe_status == "active" and  not unit:getTraits().open
				return false, "Override Remote Lockout at Security Terminal"
			end
			
	--		if unit:getTraits().security_box_locked and unit:getTraits().security_box_locked == true then 
	--			return false, "Override Draw Lockout with a Keycard"
	--		end		
